...
**NOTE: This work is to show an exploratory interaction with the data using R.**
...
### Checking out the data a bit and comparing two methods
...
```
Going to checkout the mean values with 20% trim
and then compare that to the total sum values of each year.
The question is for total, but I want to explore the data.
...
## draw a line with variable s08m to show that values in charts are equal even though the charts may not look like they do
...
Basically the same question but only using one fips ID
...
# thinking about percent change vs total volume change?
...
Los Angeles County has seen the greatest changes over time by Total Emissions, but Baltimore has the bigger change by percent for motor vehicle emissions.
